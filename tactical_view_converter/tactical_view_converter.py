# Imports standard OS module for file and path operations.
import os

# Imports system-specific parameters and functions
import sys

# Imports object-oriented filesystem path handler.
import pathlib

# Imports NumPy for fast array handling and coordinate matrix operations.
import numpy as np

# Imports OpenCV for image processing and homography functions.
import cv2

# Imports deepcopy to duplicate keypoint structures without modifying originals.
from copy import deepcopy

# Imports the custom Homography class from the local package to project 2D coordinates.
from .homography import Homography

# Resolves the absolute directory path where this current script resides.
folder_path = pathlib.Path(__file__).parent.resolve()

# Adds the parent directory to Python's module search path so utility modules can be loaded.
sys.path.append(os.path.join(folder_path, "../"))

# Imports bounding box base position and Euclidean distance calculation utilities.
from utils import get_foot_position, measure_distance

# Defines a class that projects frame pixel coordinates onto a 2D 2D top-down basketball court model.
class TacticalViewConverter:
    # Initializes court reference dimensions and keypoint mapping coordinates.
    def __init__(self, court_image_path):
        # Stores the file path to the 2D tactical court image template.
        self.court_image_path = court_image_path
        # Sets the pixel width of the target 2D tactical court image.
        self.width = 300
        # Sets the pixel height of the target 2D tactical court image.
        self.height = 161
        # Standard basketball court length in meters.
        self.actual_width_in_meters = 28
        # Standard FIBA basketball court width in meters.
        self.actual_height_in_meters = 15
        # Constructs target (x, y) coordinates on the 2D court image corresponding to standard court landmarks.
        self.key_points = [
            # left edge
            (0, 0),
    ...
    def validate_keypoints(self, keypoints_list):
        ...
        # Deep copies keypoint data to prevent mutating original inputs.
        keypoints_list = deepcopy(keypoints_list)

        # Loops through keypoint predictions frame by frame.
        for frame_idx, frame_keypoints in enumerate(keypoints_list):

            # Converts Keypoints object tensor into a list of (x, y) points.
            frame_keypoints = frame_keypoints.xy.tolist()[0]

            # Filters out undetected keypoints positioned at origin (0, 0).
            detected_indices = [i for i, kp in enumerate(frame_keypoints) if kp[0] > 0 and kp[1] > 0]

            # Skips proportion validation if fewer than 3 keypoints are present.
            if len(detected_indices) < 3:
                continue

            # List to track indices of misaligned keypoints in current frame.
            invalid_keypoints = []

            # Iterates through valid keypoint indices.
            for i in detected_indices:

                # Double-check guard for valid keypoint coordinates.
                if frame_keypoints[i][0] == 0 and frame_keypoints[i][1] == 0:
                    continue

                # Ensures at least two other non-flagged keypoints exist for distance ratio comparison.
                ...
                if t_ij > 0 and t_ik > 0:
                    ...
                    # Computes distance ratio in video frame vs. 2D court model.
                    prop_detected = d_ij / d_ik if d_ik > 0 else float('inf')
                    prop_tactical = t_ij / t_ik if t_ik > 0 else float('inf')

                    # Computes relative error between video scale ratio and true tactical model ratio.
                    error = (prop_detected - prop_tactical) / prop_tactical
                    error = abs(error)

                    # Zeroes out keypoints whose perspective distortion exceeds the 80% error threshold.
                    if error > 0.8:
                        keypoints_list[frame_idx].xy[0][i] *= 0
                        keypoints_list[frame_idx].xyn[0][i] *= 0
                        invalid_keypoints.append(i)

        # Returns sanitized court keypoint dataset with false positives suppressed.
    ...
    def transform_players_to_tactical_view(self, keypoints_list, player_tracks):
        ...
        # List to store converted 2D player coordinates for every video frame.
        tactical_player_positions = []

        # Iterates through frames pairing court keypoint detections with tracked player bounding boxes.
        for frame_idx, (frame_keypoints, frame_tracks) in enumerate(zip(keypoints_list, player_tracks)):

            # Frame dictionary mapping player IDs to top-down 2D coordinates.
            tactical_positions = {}

            # Converts frame keypoints tensor to a standard list.
            frame_keypoints = frame_keypoints.xy.tolist()[0]

            # Skip frames if keypoint list is missing or empty.
            ...
            # Builds source (video frame pixels) and target (2D court template pixels) coordinate matrices.
            ...
            try:
                ...
                for player_id, player_data in frame_tracks.items():
                    ...
                    # Assigns mapped top-down (x, y) coordinates to the corresponding player ID.
            ...
            # Appends frame's 2D player mapping dictionary to final sequence.
            tactical_player_positions.append(tactical_positions)

        # Returns list of top-down player positions for all processed video frames
